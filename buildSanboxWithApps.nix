...
let
  appsLocations =
    lib.lists.foldl (list: app: "${list} ${app}/${app.pname}") "" apps;

  # TODO: Figure this thing out
  knownApps = lib.foldl (set: subset: set // subset) { } (builtins.map (app:
    if builtins.pathExists (app + ./app-manifest.json) then {
      "webshell:app:../${app.pname}" =
        builtins.fromJSON (builtins.readFile "${app}/app-manifest.json");
    } else
      { }) apps);
